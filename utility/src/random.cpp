...
vpl::utility::RandomState vpl::utility::CreateRandomState()
{
	RandomState rs;
...
vpl::utility::RandomState vpl::utility::CreateRandomState(uint32_t seed)
{
	RandomState rs;
...
float vpl::utility::RandomFloat(RandomState& randomState, float min /* = -1.0f */, float max /* = 1.0f */)
{
	float value = randomState.floatDistribution(randomState.mersenneTwister);
	return min + value * (max - min);
}

int32_t vpl::utility::RandomInt(RandomState& randomState, int32_t min /* = -1 */, int32_t max /* = 1 */)
{
...
void vpl::utility::Seed(RandomState& randomState, uint32_t seed)
{
